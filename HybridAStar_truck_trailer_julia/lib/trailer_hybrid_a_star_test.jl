...
const XY_GRID_RESOLUTION = 0.5 #[m]
const YAW_GRID_RESOLUTION = deg2rad(10.0) #[rad]
...
const JACKKNIF_COST= 100.0 # Jackknif cost
...
function map_initial(gx::Float64, gy::Float64, gyaw::Float64, gyaw1::Float64,ox::Array{Float64}, oy::Array{Float64},xyreso::Float64, yawreso::Float64,)
    """
...
function calc_hybrid_astar_path(sx::Float64, sy::Float64, syaw::Float64, syaw1::Float64,
    ...
    @time while true
        ...
        # end
        # count += 1
        # println("keys: ", keys(open))
        # println("pq: ", pq)
        # println("pq: ", length(pq))
        if length(open) == 0
        ...
        # println("nmotion: ", nmotion)
        # quit()
        for i in 1:nmotion
            # println("current.xind: ", current.xind, "  node.yind: ", current.yind, "  current.cost: ", current.cost, "  c_id: ", c_id, "  u[i]: ", u[i], "  d[i]: ", d[i])
            node = calc_next_node(current, c_id, u[i], d[i], c)
            # println("node.xind: ", node.xind, "  node.yind: ", node.yind, "  node.cost: ", node.cost)
...
function analystic_expantion(n::Node, ngoal::Node, c::Config, ox, oy, kdtree)
    ...
    for path in paths
        ...
        # println("yaw1: ", yaw1)
        # println("cost: ", calc_rs_path_cost(path, yaw1))
    end
