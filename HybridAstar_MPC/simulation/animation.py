...
def show_animation(path, oox, ooy, sx, sy, syaw0, syaw1, gx, gy, gyaw0, gyaw1, realx, realy, real_yaw, real_speed, real_steering_angle, real_throttle, real_cte, real_epsi):
    # plt.plot(oox, ooy, ".k")
    ...
    index = []
    for i in range(1,len(realx)+1):
        index.append(i)
    x = path.x
    print("The length of x is", len(x))
    print("x type is", type(x))
    print("The length of real_yaw is", len(real_yaw))
    print("real_yaw type is", type(real_yaw))
    y = path.y
    yaw = path.yaw
    yaw1 = path.yaw1
    ...
    for ii in range(len(x)):
        plt.cla()
        plt.plot(oox, ooy, ".k")
        plt.plot(x, y, "-r", label="Hybrid A* simulated path")
        plt.plot(realx, realy, "-b", label="Real path")
    ...
    plt.figure(2)
    plt.subplot(211)
    plt.plot(index,real_steering_angle)
    plt.title("Real-time Steering angle(radian)",fontsize=20)
    plt.subplot(212)
    plt.plot(index,real_throttle)
    plt.title("Real-time Throttle",fontsize=20)

    plt.figure(3)
    plt.subplot(211)
    plt.plot(index,real_cte)
    plt.title("Real-time Cross Track Error(m)",fontsize=20)
    plt.subplot(212)
    plt.plot(index,real_epsi)
    plt.title("Real-time Orientation Error(radian)",fontsize=20)
...
def main():
    x, y, yaw, yaw1, direction = np.loadtxt('simulatedpath.dat', delimiter=' ', unpack=True)
    ...
    realx, realy, real_yaw, real_speed, real_steering_angle, real_throttle, real_cte, real_epsi = np.loadtxt('realpath.dat', delimiter=' ', unpack=True)
    show_animation(path, oox, ooy, sx, sy, syaw0, syaw1, gx, gy, gyaw0, gyaw1, realx, realy, real_yaw, real_speed, real_steering_angle, real_throttle, real_cte, real_epsi)
