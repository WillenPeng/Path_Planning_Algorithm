...
double get_ref_v(int direction, double radius){
  double v_out = get_path_length()/get_time_delta()*1.0/3.0;
...
vector<vector<double>> real_path(vector<double> path_x, vector<double> path_y, vector<double> path_yaw, vector<bool> path_direction) {
  ...
  //Modify Path Direction
  for(int i=1;i<path_direction.size()-1;++i){
    if(path_direction[i] == path_direction[i-1] && path_direction[i] != path_direction[i+1]){
      path_direction[i] = path_direction[i+1];
    } 
  }
  ...
  vector<double> steering_angle_total;
  vector<double> throttle_total;
  vector<double> cte_total;
  vector<double> epsi_total;
  ...
  while(1){
    min_dist = 999999;
    curr_direction_binary = path_direction[curr_idx];
    ...
    curr_idx_set.clear();
    count = 0;
    if(path_direction[curr_idx]) curr_direction = 1;
    ...
    for(int i = 0;i<path_x.size();++i){
      ...
      if(dist_temp<min_dist){
        min_dist = dist_temp;
        min_idx = i;
        if(min_idx == curr_idx + 1) break;
      }
      // cout<<"min_dist = "<<min_dist<<endl;
      // cout<<"min_idx = "<<min_idx<<endl;
    }
    if(min_idx<curr_idx || abs(min_idx - curr_idx)>5) min_idx = curr_idx;

    if(min_dist>5) break;

    if(count_total>2000){
      cout<<"Exceed Computation Limit!!"<<endl;
      break;
    }
  ...
  // }
    ...
    v_total.push_back(v);
    steering_angle_total.push_back(steering_angle);
    throttle_total.push_back(throttle);
    cte_total.push_back(cte);
    epsi_total.push_back(epsi);
  ...
  real_path.push_back(steering_angle_total);
  real_path.push_back(throttle_total);
  real_path.push_back(cte_total);
  real_path.push_back(epsi_total);
